...
if __name__ == "__main__":
    ...
    if config.preprocessing:
        ...
        dataset.preprocess_dataset3("lmd_matched", "lmd_matched_h5",
                                    early_exit=config.preprocessing_params["early_exit"])
        dataset.count_genres(config.preprocessing_params["prep_dataset_path"],
                             max_genres=config.model_params["s_length"])
        dataset.create_batches(batch_size=config.training_params["batch_size"])
        dataset.extract_real_song_names("lmd_matched", "lmd_matched_h5",
                                        early_exit=config.preprocessing_params["early_exit"])
    else:
        print("Initialising MusAE")
        aae = MusAE_GM(**config.model_params, **config.midi_params, **config.general_params, **config.training_params)

        print("Training MusAE")
